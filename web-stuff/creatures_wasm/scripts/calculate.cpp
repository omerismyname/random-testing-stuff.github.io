...
float* calculate(char mode[], uint8_t data[], int len) {
  float *buf = (float*) data;
  srand(time(NULL));
  for (int i = 0; i < len / 4; i+=6) {
    float newDX, newDY;
...
//emcc scripts/calculate.cpp -o scripts/calculate.js -O2 -s WASM=1 -s EXPORTED_FUNCTIONS="['_main','_calculate']" -s EXPORTED_RUNTIME_METHODS="['ccall','cwrap','getValue']"
//emcc scripts/calculate.cpp -o scripts/calculate.js -s WASM=1 -s "EXPORT_NAME='wasmCalc'" -s "EXPORTED_FUNCTIONS=['_main','_calculate']" -s EXPORTED_RUNTIME_METHODS="['ccall','cwrap']"