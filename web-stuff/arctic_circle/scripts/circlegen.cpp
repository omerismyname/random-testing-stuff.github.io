...
uint8_t iterateCircle(uint8_t buf[], uint8_t A, uint8_t currentA, uint8_t seed = 1) {
  srand(seed);
  removeClashingTiles(buf, A);
  if (processMovement(buf, A, currentA) == 0) {
    printf("AAAAAAAAAHHHH IT'S ALL FALLING APART - %d/%d\n", currentA, A);
...
//emcc scripts/circlegen.cpp -o scripts/circlegen.js -O2 -s WASM=1 -s EXPORTED_FUNCTIONS="['_main','_generateCircle','_iterateCircle','_malloc','_free']"