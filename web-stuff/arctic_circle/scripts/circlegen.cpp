...
int fillNewSquares(uint8_t* buf, int A) {
  for (int y = 0; y < 2*A; y++) {
    int off = A-y-1;
    if (off < 0) off = abs(off + 1);
    for (int x = off; x < 2*A-off; x++) {
    ...
    if (target1 > 0 && target1 < 5) {
      ...
      for (int i = 0; i < 6; i+=2) {
        ...
        if (target2 > 0 && target2 < 5) {
          moveTile(buf, x+possibleLocations[i], y+possibleLocations[i+1], A, target2);
        }
...
int processMovement(uint8_t* buf, int A, int currentA) {
  for (int y = 1; y < 2*currentA-1; y++) {
    for (int x = 1; x < 2*currentA-1; x++) {
      int bx = (A-1)+x;
      int by = 2*A*((A-1)+y);
      int d = buf[2*A*by + bx];
      if (d > 0 && d < 5) {
  ...
  for (int y = 0; y < A*2; y++) {
    for (int x = 0; x < A*2; x++) {
      ...
      int by = 2*A*((A-1)+y);
      int d = buf[2*A*by + bx];
      if (d > 0 && d < 9) buf[2*A*by + bx] = ((d-1) % 4) + 1;
    }
  }
  return 1;
}
